...
snow_depth_diff <- Smodelsimpl24 - Smodelsimpl
```
...
values(res) <- NA #initialize
res <- stack(res)
names(res) <- c("snowdepth24diff")

# Then store the estimates (predictions)
values(res$snowdepth24diff)[take] <- snow_depth_diff
```

```{r}
plot(res$snowdepth24diff, col = diverge_hcl(51, h = c(260, 305),power = 1), main = sprintf("Daily mean temperature simple model %s", strftime(date, "%b %d")))
```
